            ...
            <div class="display-dashboard">
                <form action="volunteeringservice.php" method="post">
                    ...
                    <div class="container-fluid cdiv1 special-bottom-margin" >
                        ...
                        <h3>Opportunities you are currently applying for.</h3>
                        <table class="table">
                            <thead>
                                <tr>
                            ...
                            <tbody>
                                <?php
                                    //select query:
                                    $sql = "SELECT * FROM `StudentVoluntaryService` WHERE studentID='$pid'";
                                    ...
                                    if($results->num_rows>0){
                                        ...
                                        while($row = $results->fetch_assoc()) {
                                            ...
                                            ?><td colspan='2'> 
                                                <!-- getting specified record to be updated by id. -->
                                                <a href="volunteeringservice.php?removeid=<?php echo $row['serviceID'];?>" class='btn btn-danger'
                                    ...
                                    if(isset($_GET['selectid'])){
                                        $serviceid=$_GET['selectid'];

                                        //check if student already selected the course
                                        $sql = "SELECT * FROM `StudentVoluntaryService` WHERE studentID=$pid AND serviceID=$serviceid";
                                        $results= $conn->query($sql);
                                        
                                        if($results->num_rows!=1){
                                            //no record exist, insert new selection
                                            $sql = "INSERT INTO `StudentVoluntaryService`(`studentID`, `serviceID`) VALUES ($pid,$serviceid)";
                                            if ($conn->query($sql) === TRUE) {
                                                echo "<p style='color:green; margin-left:60px;'>New service added</p>";
                                            } else {
                                                echo "Error: " . $sql . "<br>" . $conn->error;
                                            }
                                        }else{
                                            echo "<p style='color:red; margin-left:60px;'>Service already exist</p>";
                                        }
                                    }

                                    if(isset($_GET['removeid'])){
                                        $serviceid=$_GET['removeid'];
                                        
                                        // sql to delete a record
                                        $sql = "DELETE FROM `StudentVoluntaryService` WHERE studentID=$pid AND serviceID=$serviceid";

                                        if ($conn->query($sql) === TRUE) {
                                            echo "<p style='color:red; margin-left:60px;'>One service removed</p>";
                                        } else {
